...
def adminlogin():
    ...
    if request.method == 'POST':
        if request.form['uname'] == 'admin' and request.form['password'] == 'admin':
            ...
            conn = mysql.connector.connect(user='root', password='', host='localhost', database='1stressdb')
            cur = conn.cursor()
...
def check():
    ...
    conn = mysql.connector.connect(user='root', password='', host='localhost', database='1stressdb')
    cursor = conn.cursor()
    cursor.execute("SELECT * from temptb where regno='" + username + "'")
    data = cursor.fetchone()
    if data is None:
...
def emotion():
    if request.method == 'POST':
        ...
        data = cur.fetchall()
        if session['mod'] == "Happy":
            flash('You are Feeling Stress Free and Everything Feels light and joyful with no overwhelming pressure..!')
            return render_template('FacultyHome.html', data=data)
        else:
            flash('Your Are Stress')
            return render_template('ExamAnswer.html', data=data)
...
def sanswer():
    if request.method == 'POST':
        ...
        sentiment_dict = mlp.polarity_scores(Answer1)
        ...
        sentiment_dict = mlp.polarity_scores(Answer2)
        if sentiment_dict['compound'] >= 0.05:
            string = "Positive"
            m2 = 0
        elif sentiment_dict['compound'] <= - 0.05:
        ...
        sentiment_dict = mlp.polarity_scores(Answer3)
        if sentiment_dict['compound'] >= 0.05:
            string = "Positive"
            m3 = 0
        elif sentiment_dict['compound'] <= - 0.05:
        ...
        sentiment_dict = mlp.polarity_scores(Answer4)
        if sentiment_dict['compound'] >= 0.05:
            string = "Positive"
            m4 = 0
        elif sentiment_dict['compound'] <= - 0.05:
        ...
        sentiment_dict = mlp.polarity_scores(Answer5)
        if sentiment_dict['compound'] >= 0.05:
            string = "Positive"
            m5 = 0
        elif sentiment_dict['compound'] <= - 0.05:
